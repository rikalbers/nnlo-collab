...
!********************************************************************
  ...
  smin = kin%smin(jj)
  call ranvar_gnrt( mdl ,rvr ,discard,wrks(jj) ,smin,smax )
  if (discard) return
...
!********************************************************************
  ...
  smin = kin%smin(jj)
  call ranvar_wght( mdl ,rvr ,weight,wrks(jj) ,smin,smax )
  end subroutine
!
!
...
!********************************************************************
  ...
  if     (tmax.gt.dabs(tmin)) then
    ...
    t1 = tmax
    call ranvar_gnrt( mdl ,rvr ,discard,wrks(jj) ,t0,t1 )
...
!********************************************************************
  ...
  if     (tmax.gt.dabs(tmin)) then
    t0 = max( tmin ,kin%smin(jj) )
    t1 = tmax
    call ranvar_wght( mdl ,rvr ,weight,wrks(jj) ,t0,t1 )
  elseif (tmin.lt.-dabs(tmax)) then
    t0 = tmin
    t1 = min( tmax ,kin%smin(jj) )
    call ranvar_wght( mdl ,rvr ,weight,wrks(jj) ,t0,t1 )
