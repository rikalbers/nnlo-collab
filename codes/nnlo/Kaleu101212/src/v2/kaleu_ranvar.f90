...
! Batch size and maximum number of channels for grids
  integer ,private :: nbatch_grid=1000 ,nchmax=200
! Fraction of channels to be fused in a grid during optimization
  real(kind(1d0)) ,private :: fusefrac=0.00501d0 ! =2d0/(2*nchmax-1)
...
!
!
  subroutine ranvar_global( nbatch_g,nchmax_g,fusefrac_g ,nbatch_c )
!********************************************************************
!********************************************************************
  implicit none
  integer        ,optional,intent(in) :: nbatch_g,nchmax_g ,nbatch_c
  real(kind(1d0)),optional,intent(in) :: fusefrac_g
  if (present(nbatch_g  )) nbatch_grid = nbatch_g
  if (present(nchmax_g  )) nchmax      = nchmax_g
  if (present(fusefrac_g)) fusefrac    = fusefrac_g
  if (present(nbatch_c  )) nbatch_cut  = nbatch_c
  end subroutine
!
!
...
!
  subroutine ranvar_init_grid( obj ,aa )
!********************************************************************
!* Initialization
!********************************************************************
  implicit none
  type(ranvar_type) ,intent(inout) :: obj
  character(8)      ,intent(in)    :: aa
  real(kind(1d0)) :: fusefrac
!
  call grid_init( obj%grid ,nbatch_grid,nchmax,fusefrac ,aa )  
  end subroutine
!
!
...
!********************************************************************
  ...
  real(kind(1d0)) :: xx ,s0a,s1a,s0b,s1b ,wgrid
!
  if (dabs(slow).lt.supp) then
    s0b = slow
    s1b = smid
...
!
  if (obj%abov) then
    ...
    call model_wght_below( model,obj%fl ,ww,xx ,ss,s0b,s1b )
    if (xx.le.0d0.or.1d0.le.xx) then
      ww = 0d0
      return
    endif
    call grid_wght( obj%grid ,wgrid ,xx )
    ww = ww*wgrid/obj%prob
  endif
  end subroutine
!
!
...
!********************************************************************
  ...
  real(kind(1d0)) :: hh
  call grid_collect( obj%grid ,weight ,-1d0 )
  if (obj%abov) then
