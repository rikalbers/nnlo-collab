...
!
  type(model_type) :: mdl
  type(kinem_type) , dimension(:) , allocatable :: kin
  type(kaleu_type) , dimension(:) , allocatable :: obj
...
!
  integer :: istat
!
  allocate(kin(numproc),stat=istat)
  if (istat.ne.0) then
    print *,"Kaleu array kin is not allocated properly..."
    stop
  end if
