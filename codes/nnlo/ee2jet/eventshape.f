...
C===================================================================
      ...
      Subroutine Thrust_missing_mom(Pext,np,dirs,Miss)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      ...
      real*8    Pext(4,np),p(4,NpMax),dirs(4,3)
      real*8    pmiss(4),psum,pmax
      Logical   Miss
...
C-----------------------------------------------------------------------
      ...
      Integer   np,i,j,npt
      real*8    p(4,np),pt(4,NpMax),dirs(4,3),ptot,temp(4
     $     ,NpMax)
      real*8    t,dt(3),major,dmajor(3),minor,dminor(3)
...
C-----------------------------------------------------------------------
      ...
      Integer   np
      real*8      p(4,np),t,dt(3)

      real*8      px,py,pz,temp
...
C-----------------------------------------------------------------------
      ...
      Integer n
      real*8    pp(4,n),t,dt(3)

      Integer NpMax
      Parameter (NpMax=200)

      Integer ktbi,i,j,l,iter
      real*8    vcx,vcy,vcz,vlx,vly,vlz,vnx,vny,vnz,vmx,vmy
     $     ,vmz
      real*8    vmax,vsum,vnew,qtbox,qtboy,qtboz
      real*8    qtbix(NpMax),qtbiy(NpMax),qtbiz(NpMax)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      ...
      real*8 PP(4,*),DIRS(4,*),P(NMX,5),TDI(3),TPR(3),ROT(3,3)
     $     ,PV(3)
...
C***********************************************************************
      ...
      Subroutine CDparameters(p,n,Cpar,Dpar)
C-----------------------------------------------------------------------
C Calculate the C- and D-parameters
C =================================
C
...
C-----------------------------------------------------------------------
      ...
      real*8    p(4,*),txx,tyy,tzz,txy,txz,tyz,sum,pi
      real*8    Cpar,Dpar
      Integer n,i
...
C-----------------------------------------------------------------------
      ...
      real*8     p(4,*),dir(3),mh2,ml2,md2,ppos(4),pneg(4),s
     $     ,mpos,mneg
      Integer  np,i
...
C-----------------------------------------------------------------------
      ...
      real*8     p(4,*),dir(3),Btot,Bw,ptpos,ptneg,pt(3),Ptot
      Integer  np,i
...
C -- Internal Variables
      real*8    P(4,NpMax),Y(NpMax,NpMax),Ymin
      real*8    P2i,P2j,Cij,Evis,EnClus,PnClus
...
C                          EvS(7) = Mass Difference
C                          EvS(8) = Total Jet Broadening
C                          EvS(9) = Wide Jet Broadening
C                          EvS(10)= Durham y23
C                          EvS(11)= D-Parameter
C                          EvS(12)= Durham y34
C                          EvS(13-20) not yet assigned
C
C         R*4 ThD(4,3)     Thrust and Major Directions, see subr Thrust_missing_mom
C         I*4 Error        Error = 0 : o.k.
...
C -- External variables
      real*8    Pext(4,*),EvS(20),ThD(4,3)
      Integer Np,Error
      Logical Miss

C -- Internal Variables
      real*8    Dirs(4,3),ThruD(3),ExChange(4)
      real*8    Thrus,Major,Minor,Oblat,Cpar,Dpar,mh2,ml2,md2
     $     ,Btot,Bw,Yn(NpMax)
      Integer i,j
...
C -- first Thrust and related quantities
      call Thrust_missing_mom(Pext,Np,Dirs,Miss)
...
C -- C- and D-parameters
      call CDparameters(Pext,Np,Cpar,Dpar)

C -- Heavy-Jet Mass
      call JetMass(Pext,Np,ThruD,mh2,ml2,md2)

C -- Jet Broadening
      call JetBroad(Pext,Np,ThruD,Btot,Bw)

C -- Durham jet rates, E scheme
      do i=1,NpMax
         yn(i) = 0d0
      enddo
      call DurhamYn(Pext,Np,1,Yn,Error)
