...
!*****I2 - I12 + I11bare + I1100 fit*****
 ...
 subroutine Ifit3jet(y13, y23, finite, laurent)

  implicit none

!parameterlist
  real(kind(1d0)), intent(in) :: y13, y23
...
!eps^-4
    laurent(-4) = -16.055559436100037d0

!eps^-3

    laurent(-3) = -38.9586776290878d0 &
     - 0.000141755295107959d0*(log(1 - y13) + log(1 - y23)) &
     - 1.88871985560991d0*log(1 - y13 - y23) &
     + 16.9998768439114d0*(log(y13) + log(y23)) &
     + 0.0000672400062403655d0*log(y13 + y23)

!eps^-2

    laurent(-2) =  21.5386325434818d0 &
     - 1.06997380431303d0*(log(1 - y13) + log(1 - y23)) &
     + 13.8117390251145d0*(log(1 - y13)**2 + log(1 - y23)**2) &
     - 40.4632809310656d0*log(1 - y13 - y23) &
     - 36.9717798186787d0*(log(1 - y13) + log(1 - y23))*log(1 - y13 - y23) &
     - 0.745234453208208d0*log(1 - y13 - y23)**2 &
     + 0.6272206141574d0*log(y13)*log(y23) &
     + 79.0042797455575d0*(log(y13) + log(y23)) &
     + 37.94193157417d0*log(1 - y13 - y23)*(log(y13) + log(y23)) &
     - 19.6799422899321d0*(log(y13)*log(1 - y23) + log(1 - y13)*log(y23)) &
     - 13.0714758007641d0*(log(y13)**2 + log(y23)**2) &
     - 55.5046408258214d0*log(y13 + y23) &
     + 7.05806924320949d0*(log(y13) + log(y23))*log(y13 + y23) &
     - 39.7546104919914d0*log(y13 + y23)**2

!eps^-1

    laurent(-1) = 618.042956938537d0 &
     + 160.659908876448d0*(log(1 - y13) + log(1 - y23)) &
     - 150.527968173921d0*(log(1 - y13)**2 + log(1 - y23)**2) &
     - 22.5639168950242d0*(log(1 - y13)**3 + log(1 - y23)**3) &
     - 117.076938227195d0*log(1 - y13 - y23) &
     + 112.830943543426d0*(log(1 - y13) + log(1 - y23))*log(1 - y13 - y23) &
     + 42.4629010481195d0*(log(1 - y13)**2 + log(1 - y23)**2)*log(1 - y13 - y23) &
     - 48.0158936379949d0*log(1 - y13 - y23)**2 &
     - 9.06025868258899d0*(log(1 - y13) + log(1 - y23))*log(1 - y13 - y23)**2 &
     - 3.43434925565526d0*log(1 - y13 - y23)**3 &
     + 73.6980548727917d0*log(y13)*log(y23) &
     + 132.094137978339d0*(log(y13) + log(y23)) &
     + 27.0266869938221d0*log(1 - y13 - y23)*(log(y13) + log(y23)) &
     + 19.9069309059135d0*log(1 - y13 - y23)**2*(log(y13) + log(y23)) &
     + 91.2020172893416d0*(log(y13)*log(1 - y23) + log(1 - y13)*log(y23)) &
     + 8.44755081294659d0*log(1 - y13 - y23)*(log(y13)*log(1 - y23) + log(1 - y13)*log(y23)) &
     - 9.32169859887154d0*(log(y13)*log(1 - y23)**2 + log(1 - y13)**2*log(y23)) &
     - 71.6443516548326d0*(log(y13)**2 + log(y23)**2) &
     - 62.168638009123d0*log(1 - y13 - y23)*(log(y13)**2 + log(y23)**2) &
     + 45.8201949808128d0*(log(y13)**2*log(1 - y23) + log(1 - y13)*log(y23)**2) &
     - 11.9734923246091d0*(log(y13)**2*log(y23) + log(y13)*log(y23)**2) &
     + 7.22432432761758d0*(log(y13)**3 + log(y23)**3) &
     + 81.0078595556941d0*log(y13 + y23) &
     + 23.1145855144588d0*log(y13)*log(y23)*log(y13 + y23) &
     - 25.4579398553346d0*(log(y13) + log(y23))*log(y13 + y23) &
     + 0.165654324781851d0*(log(y13)**2 + log(y23)**2)*log(y13 + y23) &
     + 17.3562630277628d0*log(y13 + y23)**2 &
     + 7.82685667479819d0*(log(y13) + log(y23))*log(y13 + y23)**2 &
     + 29.3707552855815d0*log(y13 + y23)**3
...
!Calculate the finite part
  ...
  finite = 4751.40402912277d0 &
   - 26734.7000019428d0*(log(1 - y13) + log(1 - y23)) &
   - 45.3984286818547d0*(log(1 - y13)**2 + log(1 - y23)**2) &
   - 1220.15280776811d0*(log(1 - y13)**3 + log(1 - y23)**3) &
   - 89.1597491468554d0*(log(1 - y13)**4 + log(1 - y23)**4) &
   + 18848.9668572303d0*log(1 - y13 - y23) &
   - 4420.44208685264d0*(log(1 - y13) + log(1 - y23))*log(1 - y13 - y23) &
   - 308.935619763944d0*(log(1 - y13)**2 + log(1 - y23)**2)*log(1 - y13 - y23) &
   + 1.10660180349329d0*(log(1 - y13)**3 + log(1 - y23)**3)*log(1 - y13 - y23) &
   + 5013.74452249739d0*log(1 - y13 - y23)**2 &
   - 112.770812330348d0*(log(1 - y13) + log(1 - y23))*log(1 - y13 - y23)**2 &
   + 75.8336120147322d0*(log(1 - y13)**2 + log(1 - y23)**2)*log(1 - y13 - y23)**2 &
   + 980.139958082527d0*log(1 - y13 - y23)**3 &
   - 23.5020388593421d0*(log(1 - y13) + log(1 - y23))*log(1 - y13 - y23)**3 &
   + 30.4712963818887d0*log(1 - y13 - y23)**4 &
   + 3089.25740596303d0*log(y13)*log(y23) + 438.146810311018d0*log(y13)**2*log(y23)**2 &
   + 2854.68498600859d0*(log(y13) + log(y23)) &
   - 2323.4472290725d0*log(1 - y13 - y23)*(log(y13) + log(y23)) &
   - 3149.6620390879d0*log(1 - y13 - y23)**2*(log(y13) + log(y23)) &
   - 70.3897233505971d0*log(1 - y13 - y23)**3*(log(y13) + log(y23)) &
   + 850.55383672356d0*(log(y13)*log(1 - y23) + log(1 - y13)*log(y23)) &
   + 2214.85348089238d0*log(1 - y13 - y23)*(log(y13)*log(1 - y23) + log(1 - y13)*log(y23)) &
   + 11.0393437386219d0*log(1 - y13 - y23)**2*(log(y13)*log(1 - y23) + log(1 - y13)*log(y23)) &
   + 1987.64277524866d0*(log(y13)*log(1 - y23)**2 + log(1 - y13)**2*log(y23)) &
   + 6.44814885665065d0*log(1 - y13 - y23)*(log(y13)*log(1 - y23)**2 + log(1 - y13)**2*log(y23)) &
   + 11.4767349910952d0*(log(y13)*log(1 - y23)**3 + log(1 - y13)**3*log(y23)) &
   + 62.7523231581116d0*(log(y13)**2 + log(y23)**2) &
   + 2359.0474083572d0*log(1 - y13 - y23)*(log(y13)**2 + log(y23)**2) &
   - 7.69850059343979d0*log(1 - y13 - y23)**2*(log(y13)**2 + log(y23)**2) &
   - 2907.19589015713d0*(log(y13)**2*log(1 - y23) + log(1 - y13)*log(y23)**2) &
   + 27.1901277916884d0*log(1 - y13 - y23)*(log(y13)**2*log(1 - y23) + log(1 - y13)*log(y23)**2) &
   + 25.5434113977994d0*(log(y13)**2*log(1 - y23)**2 + log(1 - y13)**2*log(y23)**2) &
   + 2623.56966832683d0*(log(y13)**2*log(y23) + log(y13)*log(y23)**2) &
   + 73.2295326491967d0*(log(y13)**3 + log(y23)**3) &
   - 3.27286001509796d0*log(1 - y13 - y23)*(log(y13)**3 + log(y23)**3) &
   - 10.2780092099418d0*(log(y13)**3*log(1 - y23) + log(1 - y13)*log(y23)**3) &
   + 34.8478634080281d0*(log(y13)**3*log(y23) + log(y13)*log(y23)**3) &
   - 1.60050470329693d0*(log(y13)**4 + log(y23)**4) &
   - 22212.9472254871d0*log(y13 + y23) &
   + 2410.82469482546d0*log(y13)*log(y23)*log(y13 + y23) &
   - 7330.67683712957d0*(log(y13) + log(y23))*log(y13 + y23) &
   - 3485.8711103961d0*(log(y13)**2 + log(y23)**2)*log(y13 + y23) &
   - 84.8467765338088d0*(log(y13)**2*log(y23) + log(y13)*log(y23)**2)*log(y13 + y23) &
   - 62.1643379004786d0*(log(y13)**3 + log(y23)**3)*log(y13 + y23) &
   + 7448.80483494772d0*log(y13 + y23)**2 &
   - 49.7919249888506d0*log(y13)*log(y23)*log(y13 + y23)**2 &
   - 3542.33041423183d0*(log(y13) + log(y23))*log(y13 + y23)**2 &
   - 299.473917330888d0*(log(y13)**2 + log(y23)**2)*log(y13 + y23)**2 &
   + 6030.22440887743d0*log(y13 + y23)**3 &
   + 85.4471120271459d0*(log(y13) + log(y23))*log(y13 + y23)**3 &
   + 207.577263449365d0*log(y13 + y23)**4
