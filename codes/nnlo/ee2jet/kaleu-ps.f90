...
! The minimal value for the invariants is set here:
  ymin = 1d-8
! We offer the possibility to change this value via the input
! card:
  if (nnloinput("#ymin").gt.0) then
    ymin = nnloinput("ymin")
    write(*,*) "???????????????????????????????????????????????????????"
    write(*,*) "? The ymin value is set to a value according to user: ?"
    write(*,*) "               ymin = ",ymin
    write(*,*) "?????????????? Only working with Kaleu ????????????????"
    write(*,*) "???????????????????????????????????????????????????????"
  end if
