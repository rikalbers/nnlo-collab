...
implicit none
!
  integer , parameter :: nycut = 39
  character (len=3) , dimension(nycut) :: cycut = &
  ...
  real(kind(1d0)) , dimension(nycut+1) :: ycutbins = &
    (/0.005d0,0.015d0,0.025d0,0.035d0,0.045d0,0.055d0,0.065d0,0.075d0,0.085d0,0.095d0, &
...
!
! ycuts:
  call bookup_hist("sigma ycut",nycut,ycutbins)
!
!  call print_hist
!
!  stop
...
!
  passed = .false.
!
! Obtaining the cluster sequence:
  call fastjeteegenjadecluster(ptrack,ntrack)
  do iycut=1,nycut
    call fastjetgetjets(ycut(iycut),pjet,njet,jetvec)
    if (njet.eq.3) passed(iycut) = .true.
  end do
! Erease dynamical memory:
  call cleancluster()
!
  do iycut=1,nycut
    if (passed(iycut)) call fill_hist("sigma ycut",ycut(iycut),wgt)
  end do
!
